module Api::V1
	class PostsController < ApiController
		...
		def index
			...
			return render json: Status.find(params[:status_id]).posts.page(params[:page]).includes(:post_stat), include: :post_stat, status: :ok if params[:status_id]
		...
		def show
			return render json: Status.find(params[:status_id]).find_post, include: :post_stat, status: :ok if params[:status_id]
		...
		def update
			...
			if owner.id == current_user.id
				...
				post_edit = post.edits.new(filtered_params)
		...
		private
			...
			def filtered_params
				params.require(:answer).permit(:title, :body, :status_id, :user_id, :tag_ids)
			end
